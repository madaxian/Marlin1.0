...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      previous_millis_cmd = millis();
      endstops_hit_on_purpose();
      stepper_run_delta(-167,-137,10,250);
      break;
...
void calculate_delta(float cartesian[3]){
  ...
  delta[Y_AXIS] = ( SCARA_psi) * SCARA_RAD2DEG;// SCARA_theta + //       -  equal to sub arm angle (inverted motor)
...
//控制电机转x_ y_ z_度
void stepper_run_delta(float x_,float y_,float z_,int delay)
{
      digitalWrite(X_ENABLE_PIN,LOW); 
      digitalWrite(Y_ENABLE_PIN,LOW);
      digitalWrite(Z_ENABLE_PIN,LOW);
      ...
      int zloop=ceil(((float)abs(z_)/4)*3200);
      for(x=0;x<zloop;x++)
      {
